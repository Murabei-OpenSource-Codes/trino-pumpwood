...
echo "## Setting node.id:"
uuidgen > /etc/trino/uuid
UUID=$(cat /etc/trino/uuid)
echo "node.id=$UUID" >> /etc/trino/node.properties

# Unzip catalog information
unzip /catalog/catalog.zip -d /etc/trino/

# Ajust folder owner
chown -R trino:trino /etc/trino

# Return to trino user for security reasons
su trino
